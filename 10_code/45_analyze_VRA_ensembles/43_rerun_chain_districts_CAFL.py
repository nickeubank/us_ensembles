...
def join_and_evaluate_dislocation(state_fips, run):
    ...
    """
    if state_fips == '06' or state_fips == '01':
        state_points = gpd.read_file(f"../../../../Dropbox/dislocation_intermediate_files/60_voter_knn_scores/shapefiles/{state_names[state_fips]}_Matched_Points_VRA.shp")
    ...
    for run in run_list:#['0','1','2']:
        ...
        for n in graph.nodes():
            ...
            prec_to_index[int(graph.nodes[n]["OBJECTID"])] = n
        ...
        for t in ts:
            ...
            for step in range(step_size):
                ...
                bvaps[-1].append(sorted(new_partition["BVAP"].percents("BVAP")))
                hvaps[-1].append(sorted(new_partition["HVAP"].percents("HVAP")))
            
                pvec = new_partition[election_name].percents("Dem")
        
        
                state_points['current'] = state_points['prec_index'].map(dict(new_assignment))
...
fips_list = [('06',0),('06',1),('06',2), ('12',0),('12',1),('12',2)]

results = (Parallel(n_jobs=n_jobs, verbose=10)
           (delayed(join_and_evaluate_dislocation)(fips[0],fips[1]) for fips in fips_list)
          )
