...
def join_and_evaluate_dislocation(state_fips):
    ...
    state_points = gpd.read_file(f"../../../Dropbox/dislocation_intermediate_files/60_voter_knn_scores/shapefiles/{state_names[state_fips]}_Matched_Points_QGIS.shp") # THIS FILEANAME isn't quite right - need to check format values. 
    ...
    for run in ['0']:#['0','1','2']:
        ...
        for t in ts:
            ...
            for step in range(step_size):
                ...
                adlocs[-1].append((state_points["dislocate"]).mean())
                #if adlocs[-1][-1] < .05:
