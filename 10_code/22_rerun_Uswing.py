...
def join_and_evaluate_dislocation(state_fips):
    ...
    for run in ['0']:#['0','1','2']:
        ...
        newdir = f"../../../Dropbox/dislocation_intermediate_files/100_ensembles/{state_fips}_run{run}/rerun_Uswing/"
        ...
        for t in ts:
            ...
            for step in range(step_size):
                ...
                #state_points["dislocate"] = abs(state_points["KnnShrDem"] - (state_points["current"].map(pdict) - 0.0369))
                
                
                swung = [x - 0.0369 for x in list(pvec.values())]
                
                swung.sort()
                
                swung_vec.append(swung)
                
                swung_seats.append(sum([x >.5 for x in swung]))
            ...
            with open(newdir + "swungseats" + str(t) + ".csv", "w") as tf1:
                writer = csv.writer(tf1, lineterminator="\n")
                writer.writerows(swung_seats)
                
            with open(newdir + "swungvec" + str(t) + ".csv", "w") as tf1:
                writer = csv.writer(tf1, lineterminator="\n")
                writer.writerows(swung_vec)
